...
const apiId   = Number((DEFAULT_API_ID || meta.apiId || 0));
const apiHash = String((DEFAULT_API_HASH || meta.apiHash || ""));
...
// ───────────────────────────────────────────────────────────
// ───────────────────────────────────────────────────────────
// Нормализатор входных параметров: chat|channel|peer и text|message
app.use((req,_res,next)=>{
  try{
    ...
    // 3→1: message ← text|message (поддержка старых тел "text")
    if (req.body.message === undefined) {
      const t = (b.text ?? q.text ?? "").toString();
      if (t) req.body.message = t;
    }

    // 3→channel/peer: chat|channel|peer (работает и для GET, и для POST)
    const rawChat = (q.chat ?? b.chat ?? q.channel ?? b.channel ?? q.peer ?? b.peer ?? "").toString().trim();
    if (rawChat) {
      let val = rawChat.replace(/^https?:\/\/t\.me\//i, "@");
      const isUsername = val.startsWith("@");

      // Для GET-роутов оставим в query
      if (isUsername) {
        if (!q.channel) req.query.channel = val;
      } else {
        if (!q.peer) req.query.peer = val;
      }

      // Для POST /send и пр.: всегда положим в body.peer (он умеет принимать и @username, и -100id, и "me")
      if (!req.body.peer) req.body.peer = val;
    }
  }catch(_e){}
  next();
});
...
app.get('/me', async (req,res)=>{
  try{
    ...
    await client.connect();
    const me = await client.getMe();
    res.json({ ok:true, user: safeJson(me) });
